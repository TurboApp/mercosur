...
(function (window, $, undefined) {
	...
	$.html5fileupload = function html5fileupload(options, element) {
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="btn btn-info edit"><i class="fa fa-pencil" aria-hidden="true"></i></div>'
		//this.button.saving		= '<div class="btn btn-warning saving">Saving... <i class="glyphicon glyphicon-time"></i></div>';

		this.button.start		= '<div class="btn btn-success start"><i class="fa fa-play" aria-hidden="true"></i></div>';
		this.button.restart		= '<div class="btn btn-success restart"><i class="fa fa-repeat" aria-hidden="true"></i></div>';
		this.button.cancel		= '<div class="btn btn-danger cancel"><i class="fa fa-ban" aria-hidden="true"></i></div>';
		this.button.reset		= '<div class="btn btn-warning reset"><i class="fa fa-refresh" aria-hidden="true"></i></div>';
		this.button.remove		= '<div class="btn btn-danger remove"><i class="fa fa-times" aria-hidden="true"></i></i></div>';
		this.button.done		= '<div class="btn btn-success done"><i class="fa fa-check" aria-hidden="true"></i></div>';
		this.button.del			= '<div class="btn btn-danger del"><i class="fa fa-trash-o" aria-hidden="true"></i></div>';
		this.button.download	= '<a class="btn btn-warning download"><i class="fa fa-arrow-circle-down" aria-hidden="true"></i></a>';

		this.button.allstart	= '<div class="btn btn-success allstart"><i class="fa fa-play-circle" aria-hidden="true"></i> ' + (this.options.labelAllStart || '') + '</div>';
		this.button.allstop		= '<div class="btn btn-warning allstop"><i class="fa fa-stop-circle" aria-hidden="true"></i> ' + (this.options.labelAllStop|| '') + '</div>';
		this.button.alldone		= '<div class="btn btn-info alldone"><i class="fa fa-check-circle" aria-hidden="true"></i> ' + (this.options.labelAllDone|| '') + '</div>';
		this.button.allremove	= '<div class="btn btn-danger allremove"><i class="fa fa-trash" aria-hidden="true"></i> ' + (this.options.labelAllRemove|| '') + '</div>';
	...
	$.html5fileupload.prototype = {
		...
		addFile: function(file, existingFile) {
	    	...
	    	if (!empty(options.validExtensions) && $.inArray(extension, options.validExtensions) == -1) {
				...
				ele.append($('<div></div>').html(options.labelInvalid || 'El archivo no es Valido').addClass('notvalid'));
	    		...
	    		ele.append($('<div></div>').html(file.name).addClass('name'));
				ele.append($('<div></div>').html(options.labelMinFilesize || 'El tamaño del archivo es pequeño de lo indicado (' + minFilesize.size + " " + minFilesize.label +  ")").addClass('notvalid'));
				ele.append($(_self.progressbar).hide());
	    		...
	    		ele.append($('<div></div>').html(file.name).addClass('name'));
				ele.append($('<div></div>').html(options.labelMaxFilesize || 'El tamaño del archivo excede lo indicado (' + maxFilesize.size + " " + maxFilesize.label +  ")").addClass('notvalid'));
				ele.append($(_self.progressbar).hide());
			...
			reader.onloadend = function(evt) {
				var ajax	= $.ajax({
					...
					if (response.result == true) {
						...
						if (_self.options.onAfterStartSuccess) _self.options.onAfterStartSuccess.call(_self, _self);
					} else {
					...
					if (_self.options.onAfterStartFail) _self.options.onAfterStartFail.call(_self, _self);
			        ...
			        if (_self.options.onAfterStartAlways) _self.options.onAfterStartAlways.call(_self, _self);
				})
			}
			reader.readAsDataURL(file);
