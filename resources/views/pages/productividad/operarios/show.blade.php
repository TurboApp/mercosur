...
@section('title', 'Operario: '.$operario->nombre)
...
@section('breadcrump')
   @component('components.breadcrump',[
        'navigation'    =>  [ 'Inicio' => 'inicio', 'Operario' => 'operarios-produccion', $operario->nombre => '' ],
    ])
    @endcomponent()
@endsection
    ...
    <div class="row">
            ...
            <div id="oneDate" class="col-md-4 col-md-offset-2">
                <div class="input-group">
                    ...
                    <input type="text" id="fecha" name="fecha" value="{{$operario->fecha->format('j/m/Y')}}" class="form-control"  maxlength="10">
                </div>
            </div>    
    ...
    <script>
        $(function () {
            ...
            fecha.on('dp.change', function(e){ 
                ...
                table.ajax.url( "/operarios-produccion/API/{{$operario->id}}/"+date ).load();
            ...
            let table = $('#produccionTable').DataTable( {
                ...
                ajax: "/operarios-produccion/API/{{$operario->id}}",
                columns:[
                    { 
                        //TIPO
                        "data" : "coordinacion.servicio.tipo",
                    ...
                    {
                        //SERVICIO
                        "data" : "coordinacion.servicio.cliente.nombre",
                        "render" : function(data, type, row){
                            return `
                                <small class="text-muted">
                                    <i class="fa fa-calendar-o" aria-hidden="true"></i> ${row.date_humans} 
                                    - No. de servicio ${row.coordinacion.servicio.numero_servicio}
                                </small> 
                                <h4>${data}</h4>
                            `;
                        }
