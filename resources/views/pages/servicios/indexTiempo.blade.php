    ...
    $().ready(function(){
            ...
            columns:[
                {
                    ...
                    "render" : function(data, type, row){
                        return `
                           <span class="text-center yellow accent-5" style="display:block; font-size:3.5em;font-weight:500;">
                               ${data}
                            </span>
                        `;
                    }
                ...
                { 
                    ...
                    "render": function(data, type, row){
                        return `
                            <small class="text-muted">No. de servicio ${row.numero_servicio}</small> 
                            <h4 class="text-uppercase text-primary" style="margin:0;padding:0;">${data}</h4>
                            <h5 class="text-muted" style="margin:0;padding:0;">
                                <i class="fa fa-truck" aria-hidden="true"></i> ${row.transportes[0].transporte.nombre}
                            </h5>
                        `;
                    }    
                },
                ...
                {
                    "data":"inicio_maniobra",
                    
                },
                {
                    "data":"termino_maniobra",
                ...
                },
                {
                    ...
                    "data":null,
                    "render": function(data, type, row){
                        let btnInfo = `<a href="/servicios/${data.id}" class="btn btn-info btn-simple btn-icon">
                        ...
                        let btnEdit = `<a href="/servicios/${data.id}/editarProductividad/" class="btn btn-danger btn-simple btn-icon">
                                <i class="fa fa-pencil" aria-hidden="true"></i></a>`;

                        @if(auth()->user()->perfil_id == 1)
                            return btnInfo + btnResume + btnEdit;
                        @else
        ...
        $("#filtros").on('click','a',function(){
            switch ($(this).data("filter")) {
                case 'finalizados':
                        table.columns(6).search("Finalizado").draw();     
                    break;
                case 'haciendo':
                        table.columns(6).search("EN PROCESO").draw();     
                    break;
                case 'pendientes':
                        table.columns(6).search("PARA ASIGNAR|EN PAUSA", true, false, true).draw();     
                    break;
                default:
